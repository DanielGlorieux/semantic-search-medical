...
\begin{itemize}
    ...
    \item Performance : 68.7 sur STSB benchmark
    \item Vitesse : ~14,000 phrases/seconde sur CPU
\end{itemize}
...
\subsubsection{Phase 1 : Préparation des données (2 heures)}
...
\subsubsection{Phase 2 : Vectorisation et Indexation (30 minutes)}
...
\subsubsection{Phase 3 : API Backend (1 heure)}
...
\subsubsection{Phase 4 : Interface Utilisateur (1 heure)}
...
\subsubsection{Phase 5 : Évaluation et Optimisation (1 heure)}
...
\subsubsection{1. RAG avec LLM (Recommandé)}
...
\subsection{Annexe E : Résolution de problèmes}

\subsubsection{Erreur "Module not found"}
...
\subsubsection{Erreur "Index not found"}
...
\subsubsection{Erreur "doc\_id validation"}

\textbf{Solution :}
\begin{codeblock}{powershell}
# Convertir les doc_id en strings
...
\subsubsection{Mémoire insuffisante}

\textbf{Solution :}

Éditer \texttt{scripts/build\_index.py} :
\begin{codeblock}{python}
# Ligne 35 - Réduire batch_size
embeddings = model.encode(
    texts,
...
\subsection{Annexe F : Références}
...
\subsection{Annexe G : Licence et crédits}

\subsubsection{Projet}

Ce projet a été développé dans le cadre du cours \textit{Big Data avec Spark et Bases de Données Vectorielles} à l'École Centrale de Lyon.

\subsubsection{Technologies open-source}

Toutes les technologies utilisées sont open-source :

\begin{itemize}
    \item \textbf{Sentence Transformers} : Apache License 2.0
    \item \textbf{FAISS} : MIT License
    \item \textbf{FastAPI} : MIT License
    \item \textbf{Streamlit} : Apache License 2.0
    \item \textbf{PyTorch} : BSD License
    \item \textbf{Python} : PSF License
\end{itemize}

\subsubsection{Dataset MedQuAD}

Le dataset MedQuAD est du domaine public et peut être utilisé librement pour la recherche et l'éducation.
...
⚠️ \textbf{IMPORTANT} : Cette application est à but éducatif et de recherche uniquement. Elle ne remplace PAS un avis médical professionnel. Consultez toujours un médecin qualifié pour des questions de santé.
